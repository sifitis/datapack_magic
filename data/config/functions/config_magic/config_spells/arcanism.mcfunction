...
tellraw @s {"text":"","color":"gray","extra":[{"text":"[EDIT]","color":"dark_gray","clickEvent":{"action":"suggest_command","value":"/scoreboard players set ttlDPRISM config_magic "}},{"text":" ["},{"score":{"name":"ttlDPRISM","objective":"config_magic"},"color":"aqua"},{"text":"] Warmup time for Deep Prism Spell (Must be greater than 20t) "}]}
